...
달러 금액은 반드시 **미국식 표기($7B, $90k, $10M)와 원화 환산을 같이** 씁니다.
한국 청취자에게 "70억 달러"는 70억 원과 한순간에 섞입니다. `$7B (약 9.8조 원)`이면 자릿수가 바로 보입니다.

**환율은 에피소드 작성일 기준으로 반올림해서 고정**합니다. 시세를 매일 쫓지 마세요.

| | |
|---|---|
| 방송용 환율 | **1달러 = 1,400원** (이 회차 작성일 2026-08-21 시장가 약 1,386원, 반올림) |
| $1B | ≈ 1.4조 원 |
| $1M | ≈ 14억 원 |
| $1k | ≈ 140만 원 |
...
- 본문: `$7B (약 9.8조 원)` — 미국식 먼저, 원화를 괄호. 대략값이면 반드시 "약"
- 스탯 타일: 큰 숫자는 `$7B+`, 바로 아래 줄에 `약 9.8조 원`
- 차트 라벨: `$301B`와 `약 421조 원`을 병기
- 인용구 원문은 화자가 말한 그대로 두고, 호스트 문장·캡션·타일·차트에 환산을 붙인다
- 환율과 작성일을 `money.fx`에 남겨 둔다 (`usdPerKrw`, `asOf`, `spot`, `note`)
...
- [ ] 달러 금액에 미국식($7B / $90k / $10M)과 원화 환산이 병기돼 있는가
...
| 2026-08 | `nvidia-hugging-face` | 충돌형 (Clem vs Clem — 4년치 발언 vs 매각 보도) + 자기참조형 (우리가 찍은 파리 본사 인터뷰가 1차 자료) | **뉴스 페그형 회차는 세션 = 영상이 아니라 세션 = 막이 된다.** 영상 없는 세션을 위해 `.astro`의 video 블록을 `s.youtubeId &&` 조건부로 바꿨다(이제 템플릿 기본). **팩트 등급(사실/보도/소문/추정)을 JSON 본문에 직접 새겨라** — 미확정 인수 뉴스는 타일·인용구·요약 전부에 등급을 달아야 방송 사고가 안 난다($12.9B는 끝까지 '보도'다). **호스트의 기억도 검증 대상이다** — $16B(어디에도 없는 숫자), '마이크로독'(실제론 Microduck), '폴란드 로보틱스'(Pollen을 자동 자막이 오역) 전부 웹검증에서 정정됐다. **인용구는 '누가 한 말인지'까지 재검증** — 검증 패스가 출처 뒤집힘 2건을 잡았다(Poolside 주주 서한을 엔비디아 서한으로, CNBC 보도를 The Information으로 오귀속). **비교 차트는 결말별 3색 그룹(완료/진행·보도/무산)이 잘 먹힌다** — 3번째 색은 dataviz 검증기로 스냅한 #c8619e(자홍). 회색은 chroma floor에서 FAIL이라 못 쓴다 |
| 2026-08 | `tokens-are-the-new-dollars` | 예언형 (인수 발표 이틀 전 인터뷰가 인수 논리를 다 설명) | **호스트가 준 링크를 그대로 세션으로 만들지 말 것.** 링크 2개 중 1개(Collison YC 대담)는 주제 언급이 0이라 탭에서 빼고 대립 구도 인용 2개로 강등했다. 대신 호스트가 나중에 준 사내 잡담 영상이 "그래서 어떻게 하나"의 유일한 1차 자료라 3막으로 승격. **정제된 인터뷰는 이론, 사내 잡담은 실전 — 후자가 방송에서 더 잘 먹힌다.** 그리고 **yt-dlp 영상 다운로드가 403으로 막혀서** 프레임 캡처를 못 했다 → 대신 `money` 탭(스탯 타일 + 차트 3개)과 X 임베드를 만들었고, 결과적으로 슬라이드보다 나았다. 자막 다운로드는 계속 정상. **훅은 호스트 논지("화폐 vs 계급")가 아니라 인용구 한 줄이어야 한다.** 비교 구조는 2막에서 쓰고, 오프닝은 Patel 문장 하나로. 인수 이유의 한 줄은 "금광이 아니라 요금소". **Gaybrick 탭은 맨 뒤** — 토크노믹스와 인턴 이야기를 깔고 나서 인수 인터뷰를 들어야 요금소가 설명이 된다. **달러는 미국식+$원화 병기.** "70억 달러"는 70억 원과 섞인다. `$7B (약 9.8조 원)`, 환율은 작성일 기준 반올림 고정(이번 회 1달러 = 1,400원) |
