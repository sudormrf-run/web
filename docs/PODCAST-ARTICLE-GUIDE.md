...
| 2026-09-13 | `when-expertise-gets-cheap` | 자기참조형 (만든 회사가 자기 기술의 고발자) | **소재가 영상이 아니라 앤트로픽 글 4편**이라 포맷을 새로 짰다. YouTube 클립 전용인 `QuoteCard` 대신 텍스트 출처용 `SourceQuote` 컴포넌트를 만들고, `videoOverview`→`articleOverview`(임베드 없이 4개 글 요약), 세그먼트마다 스탯 타일, 클로징에 **경제·보안·사회 3분할 discuss 블록**을 넣었다. 스파인은 "전문가 노동의 희소성이 사라진다" 한 문장으로 네 글(경제 임금·보안 공격·무기 표적화·정렬 통제)을 escalation→zoom-out으로 꿰었다. **중국 앵글이 오용(증류 7개사)·무기(오픈웨이트 Kimi K3/GLM 5.2)에 걸쳐 강함**. 팩트 등급(`[사실·앤트로픽]`/`[보도]`/`[추정]`)을 타일·인용·근거에 새기고, "만든 회사의 자기 보고"라는 한계와 회의론(마케팅·검증 부재·번복)을 클로징·근거에 명시. **환율 1,350원**(9/11 종가 1,345.9, 이전 1,400에서 하향). 리서치는 **글당 에이전트 1개씩 4개 병렬** → `.context/anthropic-threat-week/research-*.md`. 함정: **딥링크(`#tab` 새로고침) 시 브라우저 앵커 점프가 패널 접힘보다 먼저 일어나 빈 화면**이 생김 → 로드 후 `scrollTo(0,0)` 재고정으로 해결. **headless-shell의 `--virtual-time-budget`은 rAF·타이머를 건너뛰어 이 스크롤을 검증 못 함** → `Google Chrome for Testing`(chromium-1228) 바이너리로 확인. **★ 초안 전면 반려 → 재작성**: 호스트가 "다 별로, 너무 어렵다"며 (1) **훨씬 쉬운 말**로 번역(증류→몰래 베끼기, 정렬→AI 통제, 편향된 추론→보고 싶은 대로 믿기, 노동소득분배율→일한 사람 몫 비율), (2) **추상 주제 대신 사건·예시로 시작**할 것을 요구. 막 제목을 논지("정교한 공격에 정교한 공격자가 필요 없다")에서 **구체 사건 헤드라인**("중국 AI에 물어봤더니 답이 클로드에서 왔다" / "예멘에선 미사일, 우크라이나에선 드론" / "게임인 줄 알고 진짜를 공격했다" / "변호사·회계사·개발자는 줄고 간호사는 는다")으로 바꿈. `deepDive.concept`을 "쉽게 말하면 —"으로 시작. **★ 2차 반려 → 탭 분해**: "사례가 다 사라졌다, 하나의 글이 하나의 탭일 필요 없다, 여러 사례를 하나씩 탭으로"라며 **4탭(글=탭) → 20탭(사례=탭)**으로 재구성 요구. `.astro`에서 `message`·`quote`·`question`·`hostLine`·`deepDive`·`evidence`·`followUps`·`transition`을 **전부 optional 가드**로 바꿔 가벼운 탭(제목+이야기+타일 1~2+질문 하나)도 가능하게 함. `scene.group`(어느 원문에서 나온 사례인지) 알약 태그 추가. 그룹: 오용·보안 8 / 무기·전쟁 4 / AI 통제 4 / 일자리 4. **탭은 많이, 각 탭은 가볍게.** 상세는 [[podcast-plain-language-story-first]]. |
| 2026-09-18 | `ai-native-company` | 예언형 변주 — 망원경(YC 통계) → 현미경(Lightfield 한 회사) → 같은 결론(“뭘 만들지 아는 것”) | **영상 기반 + 최신 scenes 포맷의 합본 템플릿**을 만들었다: `when-expertise-gets-cheap.astro`(훅 배너·타일·group·optional 가드)에 `from-chat-to-delegation.astro`의 `videoOverview`·`QuoteCard`·클립 다이얼로그를 이식. 영상 회차는 이 파일을 복사해서 시작할 것. 새 블록 `scene.versus`(두 영상이 어긋나는 지점 a/b/ask)와 `closing.discuss`를 `[{title, items}]` 배열로 일반화. **자동 자막엔 화자 구분이 없다** — 처음엔 ‘화자 추정/미확인’ 라벨을 달았지만 호스트가 “화자 구분 필요 없음, 통계·수치를 읽어준 것만으로 충분”이라고 해서 불확실한 발언은 ‘YC 파트너’로 통일했다. 다인 대담에서 화자 특정에 시간 쓰지 말 것. **훅은 호스트 선택으로 ‘숫자 3개 + 도발 인용’ 합성형**(`hook.stats` + “핑계가 뭡니까?”)이 됐다 — 통계 회차는 숫자를 훅 배너에 바로 띄운다. **X 임베드(`scene.tweets`)는 URL만 검색으로 찾으면 실제 렌더로 본문 검증까지 된다**(headless 스크린샷에서 원문 확인). **영상 앞 예고편에 본편 문장이 중복**되므로 인용 타임스탬프는 단어 단위 VTT 검색(`.context/{slug}/find.py`)에서 두 번째 출현을 쓴다. `.context/dedupe_srt.py`가 워크트리에 없어서 `.context/dedupe_vtt.py`로 대체(VTT 직접 파싱, 30초 문단). **진행자의 시황 발언도 검증 대상** — “Salesforce 회복”은 실제로 연초 대비 -6.5%, “Slack AI harness”는 정식명 Trusted Enterprise AI Harness, 로봇 벤치마크 수치(10% vs 60~70%)는 원자료와 불일치라 뺐다. 환율 **1,390원**(9/18 종가 1,389.4). |
| 2026-09-06 | `from-chat-to-delegation` | 현실 → 약속 → 신뢰 | 호스트가 간결함을 우선해 기존 템플릿을 축소하도록 요청했다. 장면당 메시지·핵심 질문 하나, 전체 인용 다섯 개로 구성하고 원문·맥락·추가 근거는 접는다. 영상 두 편에 같은 발언이 겹치므로 찬반 구도를 만들지 않는다. 미래 기능의 조건(사전 허락·예산)을 인용 옆에 보존한다. 분량은 확정 편성이 아닌 25분 제안. 후속 요청으로 각 장면에 90초 기술 포인트(작업 분해·하네스·검증)를 추가하되 원리·예시·질문은 펼쳐 읽게 했다. 인터뷰의 비전과 타사의 실제 구현, 설명용 가정을 구별한다. 스토리라인에 앞서 원본 영상 임베드와 실제 대화 내용의 짧은 불렛 요약을 먼저 보여준다. |
