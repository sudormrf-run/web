---
title: Covariant
description: AI robotics company founded by Pieter Abbeel, developing general-purpose robot AI
tags: [covariant, pieter-abbeel, rfm-1, robot-ai, logistics]
category: companies

# Authorship
createdBy:
  name: Jong Hyun Park
  email: jhpark@sudormrf.run
lastEditedBy:
  name: Jong Hyun Park
  email: jhpark@sudormrf.run
lastEditedAt: 2026-01-18
---

## Overview

Covariant is an AI robotics company founded in 2017 (originally as Embodied Intelligence), developing general-purpose AI for robots. The company focuses on logistics and warehouse automation, with deployments at major global retailers and logistics companies.

| Item | Details |
|------|---------|
| Headquarters | Emeryville, California |
| Founded | 2017 |
| Original Name | Embodied Intelligence |
| Total Funding | $222M+ |
| Key Customers | GXO Logistics, Otto Group, McKesson, Crate & Barrel |

---

## Founding Team

### Co-Founders

| Name | Role | Background |
|------|------|------------|
| Pieter Abbeel | Chairman & Chief Scientist | UC Berkeley Professor |
| Peter Chen | Former CEO | UC Berkeley PhD |
| Rocky Duan | Former CTO | OpenAI, UC Berkeley |
| Tianhao Zhang | Co-founder | UC Berkeley |

*Note: In August 2024, Amazon hired Abbeel, Chen, and Duan. Ted Stinson currently serves as CEO.*

---

## Core Technology

### Covariant Brain

General-purpose AI for robot vision, reasoning, and action:

```
[Visual Input] → Covariant Brain → [Robot Action]
                    ↓
         - Object Recognition
         - Grasp Planning
         - Error Recovery
```

### RFM-1 (Robotics Foundation Model 1)

Announced March 2024, an 8 billion parameter multimodal model:

| Feature | Description |
|---------|-------------|
| Parameters | 8B |
| Training Data | Text, images, video, robot actions, sensor data |
| Capabilities | Physical world understanding, natural language programming, self-reflection |

---

## Products & Deployments

### Target Applications

- **Logistics**: Warehouse picking and sorting
- **Retail**: Order fulfillment
- **Manufacturing**: Material handling

### Major Customers

- GXO Logistics
- Otto Group (Bonprix)
- McKesson
- Crate & Barrel
- Capacity
- Radial

**26+ customers** with active deployments

---

## Funding History

| Period | Round | Amount | Lead Investors |
|--------|-------|--------|----------------|
| 2017 | Seed | - | Lux Capital, Amplify Partners |
| 2018 | Series A | - | Samsung NEXT |
| 2020 | Series B | $40M | Index Ventures |
| 2021 | Series C-1 | $80M | Index Ventures, Radical Ventures |
| 2023 | Series C-2 | $75M | Radical Ventures, Index Ventures |

**Total Funding**: $222M+

### Amazon Deal (2024)

- Technology licensing agreement: $380M + $20M additional
- Key founders (Abbeel, Chen, Duan) joined Amazon

---

## Research Contributions

### Key Ideas

1. **General-Purpose Robot AI**: Single model for diverse tasks
2. **Real-World Learning**: Learning from actual deployment data
3. **Scalable Architecture**: Foundation model approach to robotics

### Influence on Field

- Early commercial success of robot learning
- Demonstration of foundation model viability for robotics
- Bridge between academic research and industry deployment

---

## References

- [Covariant Official Site](https://covariant.ai/)
- [Covariant Wikipedia](https://en.wikipedia.org/wiki/Covariant_(company))
- [RFM-1 Announcement](https://www.businesswire.com/news/home/20240311948570/en/Covariant-Introduces-RFM-1-to-Give-Robots-the-Human-like-Ability-to-Reason)
- [TechCrunch - Amazon Hires Covariant Founders](https://techcrunch.com/2024/08/31/amazon-hires-the-founders-of-robotics-ai-startup-covariant/)

---

## See Also

- [Company List](./)
- [Pieter Abbeel](../people/pieter-abbeel)
- [Non-Teleop Data Collection](../essays/insights/non-teleop-data)
