---
title: Unitree
description: Unitree Robotics - China's Low-Cost Robot Hardware Manufacturer
tags: [unitree, hardware, quadruped, humanoid, g1, h1, china]
category: companies

# Authorship
createdBy:
  name: Jong Hyun Park
  email: jhpark@sudormrf.run
lastEditedBy:
  name: Jong Hyun Park
  email: jhpark@sudormrf.run
lastEditedAt: 2026-01-15
---

## Overview

Unitree Robotics is a Chinese robot hardware manufacturer producing affordable yet high-performance quadruped robots and humanoids. They are leading price innovation with the $16,000 G1 humanoid.

| Item | Details |
|------|---------|
| Headquarters | Hangzhou, China |
| Founded | 2016 |
| CEO | Wang Xingxing |
| Valuation | ~$7B (estimated from reports) |
| Core Products | Go2, B2, G1, H1 |

---

## Product Lineup

### Quadruped Robots

| Model | Purpose | Price |
|-------|---------|-------|
| Go2 | Consumer/Research | ~$1,600 |
| B2 | Industrial | ~$10,000 |
| B2-W | Wheel hybrid | - |

### Humanoid Robots

| Model | Specs | Price |
|-------|-------|-------|
| G1 | 1.27m, 35kg | ~$16,000 |
| H1 | 1.8m, 47kg | ~$90,000 |
| H2 | Next-gen (2025) | TBD |

---

## G1 Humanoid

### Specs

| Item | Value |
|------|-------|
| Height | 127cm (size of an 8-year-old) |
| Weight | 35kg |
| DoF | 23-43 DoF (depending on configuration) |
| Walking Speed | 2m/s (4.5 MPH) |
| Sensors | 3D LiDAR, Intel RealSense D435 |
| Hands | Force-controllable fingers |

### Features

- **Foldable Design**: Compact for transportation
- **Affordable Price**: $16,000 (revolutionary compared to other humanoids)
- **RL-Based**: Imitation/reinforcement learning applied

---

## H1 Humanoid

### Specs

| Item | Value |
|------|-------|
| Height | 180cm |
| Weight | 47kg |
| Max Torque | 360N.m |
| Running Speed | 3.3m/s (world record) |
| Price | ~$90,000 |

### Achievements

- **World's Fastest Humanoid**: 3.3m/s running
- **World Humanoid Robot Games Gold** (4 golds in speed events)
- Autonomous walking on complex terrain

---

## Technical Features

### Imitation & Reinforcement Learning

```
[Human Motion Demo] → [Imitation Learning] → [Initial Policy]
                                                   │
                                       [RL Fine-tuning]
                                                   │
                                            [Final Policy]
```

### Hardware Strengths

| Strength | Description |
|----------|-------------|
| Price Competitiveness | Low-cost Chinese manufacturing |
| High-Performance Motors | Self-developed high-torque motors |
| Vertical Integration | End-to-end design and manufacturing |
| Mass Production | Scalable production lines |

---

## Security-Related Reports (2025)

*The following is from security researcher reports and requires independent verification.*

Potential issues reported in 2025 security research (source: security research presentations):
- Concerns raised about sensor data collection
- BLE-related vulnerabilities reported
- Affected models: Go2, B2, G1, H1 (per report)

---

## Market Strategy

### Price Innovation

```
Boston Dynamics Atlas: ~$1,000,000+
Tesla Optimus target:  ~$25,000
Unitree G1:           ~$16,000  ← Most affordable
```

### Target Markets

- **Research/Education**: Affordable research platform
- **Industrial**: B2 series for logistics/inspection
- **Consumer**: Go2 for companion robot market

---

## IPO Outlook (2025)

| Item | Details |
|------|---------|
| Expected Valuation | $7B |
| Market | Hong Kong |
| Timeline | 2025 |

---

## References

- [Unitree Robotics](https://www.unitree.com/)
- [Unitree Shop](https://shop.unitree.com/)
- [The Robot Report - G1](https://www.therobotreport.com/unitree-robotics-unveils-g1-humanoid-for-16k/)
- [Wikipedia - Unitree](https://en.wikipedia.org/wiki/Unitree_Robotics)

---

## See Also

- [Company List](index.md)
- [Humanoid Design](../deep-dives/humanoid-design.md)
- [Tesla](tesla.md)
