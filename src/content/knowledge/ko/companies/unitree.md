---
title: Unitree
description: Unitree Robotics - 중국의 저가형 로봇 하드웨어 제조사
tags: [unitree, hardware, quadruped, humanoid, g1, h1, china]
category: companies

# Authorship
createdBy:
  name: 박종현
  email: jhpark@sudormrf.run
lastEditedBy:
  name: 박종현
  email: jhpark@sudormrf.run
lastEditedAt: 2026-01-15
---

## Overview

Unitree Robotics는 중국의 로봇 하드웨어 제조사로, 저렴하면서도 고성능인 사족보행 로봇과 휴머노이드를 생산합니다. $16,000 휴머노이드 G1으로 가격 혁신을 이끌고 있습니다.

| 항목 | 내용 |
|------|------|
| 본사 | 항저우, 중국 |
| 설립 | 2016년 |
| CEO | Wang Xingxing |
| 기업가치 | ~$7B (보도 기반 추정) |
| 핵심 제품 | Go2, B2, G1, H1 |

---

## 제품 라인업

### 사족보행 로봇 (Quadruped)

| 모델 | 용도 | 가격 |
|------|------|------|
| Go2 | 소비자/연구용 | ~$1,600 |
| B2 | 산업용 | ~$10,000 |
| B2-W | 휠 하이브리드 | - |

### 휴머노이드 로봇

| 모델 | 스펙 | 가격 |
|------|------|------|
| G1 | 1.27m, 35kg | ~$16,000 |
| H1 | 1.8m, 47kg | ~$90,000 |
| H2 | 차세대 (2025) | TBD |

---

## G1 휴머노이드

### 스펙

| 항목 | 수치 |
|------|------|
| 높이 | 127cm (8세 아동 크기) |
| 무게 | 35kg |
| 자유도 | 23-43 DoF (구성에 따라) |
| 보행속도 | 2m/s (4.5 MPH) |
| 센서 | 3D LiDAR, Intel RealSense D435 |
| 손 | 힘 제어 가능한 손가락 |

### 특징

- **접이식 디자인**: 컴팩트하게 운반 가능
- **저렴한 가격**: $16,000 (기존 휴머노이드 대비 혁신적)
- **강화학습 기반**: 이미테이션/강화 학습 적용

---

## H1 휴머노이드

### 스펙

| 항목 | 수치 |
|------|------|
| 높이 | 180cm |
| 무게 | 47kg |
| 최대 토크 | 360N.m |
| 달리기 속도 | 3.3m/s (세계 기록) |
| 가격 | ~$90,000 |

### 성과

- **세계 최고속 휴머노이드**: 3.3m/s 달리기
- **World Humanoid Robot Games 금메달** (고속 종목 4관왕)
- 복잡한 지형 자율 보행

---

## 기술적 특징

### 이미테이션 & 강화학습

```
[인간 동작 데모] → [이미테이션 러닝] → [초기 Policy]
                                           │
                               [강화학습 미세조정]
                                           │
                                    [최종 Policy]
```

### 하드웨어 강점

| 강점 | 설명 |
|------|------|
| 가격 경쟁력 | 중국 제조 기반 저비용 |
| 고성능 모터 | 자체 개발 고토크 모터 |
| 수직 통합 | 설계~제조 일괄 처리 |
| 대량 생산 | 스케일 가능한 생산라인 |

---

## 보안 관련 보도 (2025)

*아래는 보안 연구자들의 보고 내용으로, 별도 검증 필요.*

2025년 보안 연구에서 보고된 잠재적 이슈 (출처: 보안 연구 발표):
- 센서 데이터 수집 관련 우려 제기
- BLE 관련 취약점 보고
- 영향 범위: Go2, B2, G1, H1 (보고 기준)

---

## 시장 전략

### 가격 혁신

```
Boston Dynamics Atlas: ~$1,000,000+
Tesla Optimus 목표:   ~$25,000
Unitree G1:          ~$16,000  ← 가장 저렴
```

### 타겟 시장

- **연구/교육**: 저렴한 연구 플랫폼
- **산업용**: B2 시리즈로 물류/검사
- **소비자**: Go2로 반려로봇 시장

---

## IPO 전망 (2025)

| 항목 | 내용 |
|------|------|
| 예상 가치 | $7B |
| 시장 | 홍콩 |
| 시기 | 2025년 |

---

## References

- [Unitree Robotics](https://www.unitree.com/)
- [Unitree Shop](https://shop.unitree.com/)
- [The Robot Report - G1](https://www.therobotreport.com/unitree-robotics-unveils-g1-humanoid-for-16k/)
- [Wikipedia - Unitree](https://en.wikipedia.org/wiki/Unitree_Robotics)

---

## See Also

- [회사 목록](index)
- [휴머노이드 설계](../essays/humanoid-design)
- [Tesla](tesla)

