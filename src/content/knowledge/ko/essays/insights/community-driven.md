---
title: 커뮤니티 드리븐 접근
description: HuggingFace LeRobot, OXE, RoboArena 등 오픈소스 커뮤니티의 노력
tags: [community, open-source, huggingface, lerobot, oxe, roboarena]
category: essays
order: 10
related:
  - non-teleop-data.md

# Authorship
createdBy:
  name: 박종현
  email: jhpark@sudormrf.run
lastEditedBy:
  name: 박종현
  email: jhpark@sudormrf.run
lastEditedAt: 2026-01-15
---

## 개요

HuggingFace는 오픈소스 정신으로 커뮤니티 드리븐 데이터를 모으고 있습니다.
학계에서는 연구실들이 뭉쳐 OXE와 같은 데이터셋을 구축하고 RoboArena와 같은 분산 평가 시스템을 조직하고 있습니다.

...

---

## HuggingFace LeRobot

### 프로젝트 소개

...

### LeRobot 해커톤

...

---

## Open X-Embodiment (OXE)

...

---

## RoboArena

분산 평가 시스템

...

---

## 희망과 한계

LeRobot 해커톤을 서울에서 호스팅하면서 느낀 커뮤니티 드리븐 방식의 희망과 문제점

...

---

## See Also

- [Non-Teleop 데이터 수집](non-teleop-data)
- [오픈소스 VLA 모델들](/knowledge/models/openvla)
- [HuggingFace](/knowledge/companies/huggingface)

